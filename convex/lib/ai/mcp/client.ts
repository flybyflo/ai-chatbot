  ...
  async connect(): Promise<boolean> {
    try {
      ...
      if (this.config.headers?.Authorization) {
        console.log("[MCP][convex client] Connecting with authorization", {
        ...
        console.log(
          "[MCP][convex client] Connecting without authorization header",
          {
            serverName: this.config.name,
            url: this.config.url,
      ...
      if (typeof (error as any)?.response === "object") {
        ...
        try {
          ...
          console.warn(
            `[MCP][convex client] Failed to read error body for ${this.config.name}:`,
            bodyError
          );
        }
        console.warn(
          `[MCP][convex client] HTTP error details for ${this.config.name}:`,
          {
